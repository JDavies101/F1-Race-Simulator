# introduce traffic (probabalistic)
...
def simulate_race(pit_lap, sc_laps, sc_base_lap_time, sc_pit_loss,
                  ...
                  base_traffic_prob, spread_rate, traffic_time_loss, pit_lap2=None):
    ...
    for laps in range(1, total_laps+1):
        ...
        traffic_prob = base_traffic_prob * np.exp(-stint_lap / spread_rate)
...
def run_simulation(args):
    ...
    for pit_lap in range(1, config['total_laps']-5):
        results_1_stop[pit_lap], lap_times_1_stop[pit_lap] = simulate_race(
            pit_lap, sc_laps,
            ...
            config['base_traffic_prob'], config['spread_rate'], config['traffic_time_loss'],
            pit_lap2=None)
        
        for pit_lap2 in range(pit_lap + 1, config['total_laps']-2):
            results_2_stop[(pit_lap, pit_lap2)], lap_times_2_stop[pit_lap] = simulate_race(
                pit_lap, sc_laps,
                config['sc_base_lap_time'], config['sc_pit_loss'],
                config['base_lap_time'], config['pit_loss'],
                config['tire_degradation_rate'], config['total_laps'],
...
def plot_single_race(config, pit_laps):
        sc_laps = generate_safety_cars(config['total_laps'], config['sc_chance'])
        ...
        plt.legend()
        plt.ylim(config['base_lap_time'], config['base_lap_time']+35)
        plt.savefig('lap_trace_v6.png', dpi=450, bbox_inches='tight')
        plt.close()
...
def main():
    ...
    if most_common_strategy == '2-stop':
        plot_single_race(config, [best_2[0], best_2[1]])
    else:
        plot_single_race(config, [best_1])
    ...
    plt.legend()
    plt.savefig('monte_carlo_dist_v6_1_stop.png', dpi=450, bbox_inches='tight')
    plt.close()
    ...
    plt.legend()
    plt.savefig('monte_carlo_dist_v6_2_stop.png', dpi=450, bbox_inches='tight')
    plt.close()
    ...
    plt.legend()
    plt.savefig('monte_carlo_dist_v6_strategy.png', dpi=450, bbox_inches='tight')
    plt.show()
